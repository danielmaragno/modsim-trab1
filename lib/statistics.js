...
var estatisticaTemposEntidades = function(server1, server2, entidades, $scope){
	...
	let totalTime = 0;
	for(var i=0, time; time=entidades['tipo1TemposSistema'][i++]; ){
		totalTime += time;
	}
	$scope.entidades.tipo1AvgSystemTime = time/entidades['tipo1TemposSistema'].length;
	console.log("Tempo Médio no Sistema (TIPO 1)", $scope.entidades.tipo1AvgSystemTime);

	totalTime = 0;
	for(var i=0, time; time=entidades['tipo2TemposSistema'][i++]; ){
		totalTime += time;
	}
	$scope.entidades.tipo2AvgSystemTime = time/entidades['tipo2TemposSistema'].length;
